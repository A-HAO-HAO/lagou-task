...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private List<String> inteceptorClassNames = new ArrayList<>(); // 缓存扫描到的interceptor的全限定类名
    ...
    private Map<Pattern,HandlerInterceptor> handlerInterceptorMap = new LinkedHashMap<>();
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // 2 扫描相关的类，扫描注解
        doScan(properties.getProperty("scanPackage"),classNames);
        ...
        //6 扫描所有拦截器
        doScan(properties.getProperty("interceptorPackage"),inteceptorClassNames);
        doScanInterceptor();
    ...
    private void doScanInterceptor(){
        for (String className : inteceptorClassNames) {
            try {
                Class<?> clazz = Class.forName(className);
                if(clazz.isAnnotationPresent(LagouInterceptor.class)){
                    LagouInterceptor annotation = clazz.getAnnotation(LagouInterceptor.class);
                    String path = annotation.path();
                    handlerInterceptorMap.put(Pattern.compile(path),(HandlerInterceptor) (clazz.newInstance()));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void doScan(String scanPackage,List<String> cont) {
        ...
        for(File file: files) {
            if(file.isDirectory()) { // 子package
                // 递归
                doScan(scanPackage + "." + file.getName(),cont);  // com.lagou.demo.controller
...
//        method.invoke() //
        ...
        //先拿到所有拦截器
        List<HandlerInterceptor> interceptor = getInterceptor(req);
        //处理所有拦截器后决定是否放行
        for (HandlerInterceptor handlerInterceptor : interceptor) {
            if(!handlerInterceptor.preHandler(req,resp,handler)){
                return;
            }
        }
    ...
    private List<HandlerInterceptor> getInterceptor(HttpServletRequest req) {
        if(handlerInterceptorMap.isEmpty()){return null;}
        List<HandlerInterceptor> interceptorList = new ArrayList<>();
        String url = req.getRequestURI();
        for(Pattern key: handlerInterceptorMap.keySet()) {
            Matcher matcher = key.matcher(url);
            if(matcher.matches()){
                interceptorList.add(handlerInterceptorMap.get(key));
            }
        }
